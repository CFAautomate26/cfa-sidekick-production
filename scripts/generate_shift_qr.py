#!/usr/bin/env python3
"""Generate the Chick-fil-A branded Shift Lead app QR code and poster.

Usage:
    python scripts/generate_shift_qr.py [APP_URL]

APP_URL defaults to the production /shift URL on Render. Outputs are written
to assets/shift-qr/:
    shift-qr.png      - branded QR code (logo centre, CFA red modules)
    shift-poster.png  - print-ready 8.5x11 poster (300 DPI) with the QR
    shift-poster.pdf  - the same poster as a printable PDF
...
DEFAULT_URL = "https://cfa-sidekick-production-4hx7.onrender.com/shift"
REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT_DIR = os.path.join(REPO_ROOT, "assets", "shift-qr")
...
def build_poster(qr_img: Image.Image) -> Image.Image:
    ...
    f_hint = load_font("Medium", 58)

    y = 90
    y = centred_text(d, y, "LEADERSHIP TEAM", f_eyebrow, WHITE, W) + 28
    centred_text(d, y, "SHIFT LEAD APP", f_title, WHITE, W)
    ...
    for line in (
        "Checklists, lineups, goals and shift notes —",
        "everything you need to run the shift,",
        "right on your phone.",
    ):
    ...
    y = centred_text(d, y, "SCAN TO OPEN", f_scan, CFA_RED, W) + 40
    y = centred_text(d, y, "Sign in with your PIN, then Add to Home Screen",
                     f_hint, SOFT, W) + 22
    centred_text(d, y, "No PIN yet? Ask the Operator.", f_hint, SOFT, W)

    foot_h = 170
    d.rectangle([0, H - foot_h, W, H], fill=CFA_RED)
    footer = ("Chick-fil-A Wharncliffe & Wonderland · "
              "3459 Wonderland Road South, London, ON")
    # Shrink to fit with a comfortable side margin.
    size = 58
    while size > 30:
        fb = load_font("Medium", size)
...
def main() -> None:
    ...
    qr_img = build_qr(url)
    qr_path = os.path.join(OUT_DIR, "shift-qr.png")
    qr_img.save(qr_path)
    poster = build_poster(qr_img)
    poster_path = os.path.join(OUT_DIR, "shift-poster.png")
    poster.save(poster_path, dpi=(300, 300))
    pdf_path = os.path.join(OUT_DIR, "shift-poster.pdf")
    poster.save(pdf_path, "PDF", resolution=300)
